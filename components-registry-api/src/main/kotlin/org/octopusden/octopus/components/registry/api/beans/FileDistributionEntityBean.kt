...
open class FileDistributionEntityBean : FileDistributionEntity {
    companion object {
                ...
                for (queryPart: String in queryParts) {
                    ...
                    if (!Arrays.stream(ARTIFACT_ATTRIBUTES).anyMatch { attribute -> queryAttributeParts[0] == attribute }) {
                        throw IllegalStateException(
                            "Unknown query attribute '" + queryAttributeParts[0] + "' in URI '" + distributionItem + "'",
                        )
                    }
                    if (!queryAttributeParts[1].matches(QUERY_ATTRIBUTE_VALUE_VALIDATION_PATTERN)) {
                        throw IllegalStateException(
                            "The query '" + queryAttributeParts[0] +
                                "' attribute value '" + queryAttributeParts[1] +
                                "' contains not allowed symbol(s) [validation pattern: '" +
    ...
    override fun getClassifier(): Optional<String> = Optional.ofNullable(classifier)

    override fun getArtifactId(): Optional<String> = Optional.ofNullable(artifactId)
