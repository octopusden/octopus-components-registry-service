...
**Known limitation of this rollup.** When RMS has data, "the maximum version ever seen" is not necessarily what the component builds on today. Example: an old, still-maintained line once used Java 21, but the current line has since moved down to Java 8. The list view shows "21," even though the component actually builds on 8 right now. Accepted trade-off, not something we missed. The `javaVersion` filter inherits it (Decision 4a), which is where it bites hardest — read that consequence before treating the list as an inventory of what runs on which Java.

**This is a semantic change to an existing field, not a new one.** Before this change, `ComponentSummaryResponse.javaVersion` was always the BASE row's configured column. It now reports RMS's value whenever RMS has one, so the same component can report a different version than it did before with no configuration change. Two consequences worth being explicit about, both accepted:

- The raw configured value is **no longer retrievable from the list endpoint at all** — there is no second field carrying it. A consumer that specifically needs "what CRS is configured to expect" (rather than "what was actually built") must read the component's detail response, where the configured rows and ACTUAL ranges are separate.
- Existing consumers see the new meaning without opting in — no new field, no version bump. The Portal is the intended consumer and renders this as the component's Java version, which is the value this change is meant to improve. Any other v4 list consumer needs to be told; see the proposal's Affected areas.

An earlier draft carried the rollup as its own `registeredBuildParameters.java` field alongside the untouched `javaVersion`, which would have avoided both consequences. It was dropped deliberately: two adjacent Java-version fields on a list row, differing only in provenance, is a worse thing to ask a UI (and a human) to disambiguate than one field with a documented, consistent meaning.
...
- This trades DB-side pagination for an in-memory pass over every other-filter-matching candidate, only when `javaVersion` is part of the request. Acceptable while that candidate set stays in the low thousands (current data volume); revisit if it grows an order of magnitude past that. Tracked as **TD-020**, which also records why the cost is independent of the requested page size.

**Matching compares major versions, not strings.** `1.8` and `8` are the same Java version everywhere else in this feature (Decision 2), so the filter uses that same comparison rather than string equality — otherwise `?javaVersion=8` could not match a value the list itself displays as `1.8`. RMS is expected to record Java in the same spellings CRS already uses (`1.8`, `17`, `21`, `25`), so in practice both sides usually agree on the literal string; the comparison is what makes that an expectation rather than a dependency, and it also covers CRS's *own* configured column, which has never been constrained to one spelling. A value neither side can parse as a Java version still matches by exact string, so a hand-typed configured value stays findable by typing it verbatim.

**The filter inherits the rollup's "maximum, not current" semantics, and that is the intended behaviour.** Because the effective value is the maximum across all ACTUAL ranges (Decision 4), `?javaVersion=` answers *"which components have ever recorded this version as their highest"* — not *"which components still build on it."* A component whose old, still-maintained line runs Java 8 while its current line runs 21 is matched by `?javaVersion=21` and **not** by `?javaVersion=8`. This is deliberate and consistent: the filter and the list column always agree, so a filtered result set never contains a row whose displayed value contradicts the query. Answering "which components are still on an old Java version" needs a per-range query, which is the detail view's job (`javaActualRanges`), not the list's.
...
**Every duration field also gets its own strictly-positive `@AssertTrue`** — `connectTimeout`, `readTimeout`, `normalInterval`, `initialRetryInterval`, `retryBackoffCap`, `sweepTimeout`, `writeGateTimeout`. A zero/negative `normalInterval` or `initialRetryInterval` makes `nextDelay()` return `Duration.ZERO`, hot-looping the sweep against RMS with no backpressure; a zero `sweepTimeout` expires before the first `Future.get`, so the sweep "succeeds" with every component unavailable and never retries faster, going permanently dark without ever surfacing an error. `writeGateTimeout` fails louder (every gated write becomes a 503) but is the same class of misconfiguration. All seven are guarded the same way `connectTimeout`/`readTimeout` originally were.
...
This entire feature — ACTUAL display, the list view's effective `javaVersion`, and the write gate — applies only to components whose (effective) build system is `MAVEN` or `GRADLE` (`BuildSystem` enum, `component-resolver-api`). For any other build system (`BS2_0`, `GOLANG`, `IN_CONTAINER`, `WHISKEY`, `PROVIDED`, `ESCROW_NOT_SUPPORTED`, `ESCROW_PROVIDED_MANUALLY`, ...), a Java/Maven version is a meaningless concept, so the feature does nothing: no RMS call for that component in the sweep, no ACTUAL data in its responses (its list-view `javaVersion` simply stays the configured value), no write gate on its `javaVersion`/`mavenVersion` fields (which, for a non-Maven/Gradle component, are unlikely to be meaningfully set in the first place).
...
**Archived components are excluded too**, on a separate axis from build system: `findNonArchivedMavenOrGradleComponentKeys` never sweeps an archived component, so its cache entry is always absent — the same shape as "eligible but never successfully swept." `RegisteredBuildParametersMapper.detailFor` checks `archived` explicitly, before the build-system check, so an archived component's detail response reports no ACTUAL data at all (`null`) rather than the misleading `actualDataUnavailable = true`, which would otherwise read as an RMS problem rather than "not applicable." The list view's `javaVersion` needs no equivalent check — it already falls back to the configured value whenever the sweep's map has no entry, for either reason.
...
### 16. Surfacing RMS data on the `as-code` (Groovy DSL) export
...
**New pure function:** `RegisteredBuildParametersMapper.actualValueAt(ranges: List<BuildRangeCollapser.Run>, target: IVersionInfo, versionRangeFactory: VersionRangeFactory): String?` — walks `ranges`, returns the value of whichever range's `versionRangeFactory.create(range.versionRange).containsVersion(target)` is true, else `null`. ACTUAL ranges are guaranteed non-overlapping by construction (`BuildRangeCollapser` produces one ordered, disjoint list), so at most one match is possible; no ambiguity to resolve.

**Wiring.** `ComponentCodeRenderer` itself has no RMS dependency today (constructed with only `versionRangeFactory`/`numericVersionFactory`) and gains none — `ComponentManagementServiceImpl` (which already holds `rmsBuildParametersService`) looks up `rmsBuildParametersService?.currentReport()?.components?.get(entity.componentKey)` once per call and passes the resulting `ComponentBuildRanges?` into `renderFull`/`renderResolved` as a new parameter, the same shape `ownershipExportPatterns(entity)` is already threaded through today. A `null` (feature disabled, ineligible component, or never swept) needs no separate eligibility check at the renderer — the sweep's cache map simply has no entry for that component in exactly those cases, so "no RMS data" and "not eligible" collapse into the same, already-handled `null` case.
...
- **The `as-code` full view's RMS section (Decision 16) is a reimport hazard, accepted deliberately.** Its per-range blocks are rendered with the exact same uncommented syntax as a real `SCALAR_OVERRIDE` row — only the section's header line is a comment. Someone exporting `as-code`, hand-editing the file, and re-committing it as the component's Git/DSL source would silently turn RMS's read-only ACTUAL data into a real configured override, contradicting this feature's core "CRS never writes ACTUAL" principle (Non-Goals). Considered commenting out every line instead (fully inert, immune to this) and rejected in favor of readability: the range blocks visually matching real override syntax is the point, and the Git/DSL import path is already out of scope for this whole feature (Out of Scope) — nothing currently feeds `as-code`'s exported text back into `ImportServiceImpl` automatically. If that ever changes, or if this hazard is hit in practice, revisit by commenting out the range lines too.
...
- **An environment with `enabled=false` gets neither the display nor the enforcement — by design.** Per Decision 6, this is intentional and safe to roll out incrementally (no writes are ever blocked in a disabled environment). An operator who never turns it on sees no ACTUAL data and no blocking, with no error to notice — mitigated by an informational (not a warning) startup log line stating the feature is disabled, so it reads as "not configured" rather than "not implemented." (This risk does *not* apply to the "turned on but forgot the URL" case — Decision 11's fail-fast validation surfaces that one loudly, at startup.)
...
- **The write gate's executor (`RMSOverrideGate`) is unbounded, by design, not oversight.** It uses `Executors.newCachedThreadPool()` with no maximum. Considered adding a cap sized like the sweep's `sweepConcurrency` and rejected: thread count here scales with how many writes need a live gate check *at the same moment*, not with the number of components in CRS (unlike the sweep, which touches every component every cycle) — realistic concurrent-write load is far below any count that would matter, and an idle thread costs nothing. Revisit only if concurrent-write volume ever grows enough to make that assumption wrong.
- **Two sweep/gate quirks, both cosmetic, both left as-is.** The sweep's shared deadline and fixed component order mean that if it routinely runs out of time, it's always the same tail of the component list that goes stale, never a rotating subset — a real but narrow unfairness under sustained overload. Separately, `future.cancel(true)` in both the sweep and the write gate does not actually interrupt `SimpleClientHttpRequestFactory`'s blocking socket read, so a cancelled call's underlying thread keeps running until its own connect/read timeout elapses — `writeGateTimeout`/`sweepTimeout` bound how long the *caller* waits, not how long the real network call takes to give up. Harmless (the orphaned thread is discarded when it eventually finishes) but worth knowing the timeout numbers aren't as tight as they look.
