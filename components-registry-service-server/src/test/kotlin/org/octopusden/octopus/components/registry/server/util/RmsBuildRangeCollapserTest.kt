...
class RmsBuildRangeCollapserTest {
    ...
    fun `empty input produces no runs`() {
        assertEquals(emptyList<RmsBuildRangeCollapser.Run>(), RmsBuildRangeCollapser.collapse(emptyList()))
    }

    @Test
    @DisplayName("a single build produces one open-ended run")
    fun `single build produces one open-ended run`() {
        val result = RmsBuildRangeCollapser.collapse(listOf(Build("2", "17")))
        assertEquals(listOf(RmsBuildRangeCollapser.Run("[2,)", "17")), result)
    }
    ...
    fun `two builds with different values produce two runs`() {
        ...
        val result = RmsBuildRangeCollapser.collapse(listOf(Build("2", "17"), Build("3", "21")))
        assertEquals(
            listOf(
    ...
    fun `same-value builds separated by an unbuilt gap merge into one run`() {
        val result = RmsBuildRangeCollapser.collapse(listOf(Build("5", "17"), Build("10", "17")))
        assertEquals(listOf(RmsBuildRangeCollapser.Run("[5,)", "17")), result)
    }
    ...
    fun `a trailing null build closes the run with no upper bound left open`() {
        val result = RmsBuildRangeCollapser.collapse(listOf(Build("1", "17"), Build("2", "17"), Build("3", null)))
        assertEquals(listOf(RmsBuildRangeCollapser.Run("[1,3)", "17")), result)
    }
    ...
    fun `custom valuesEqual merges values the collapser would otherwise treat as different`() {
        ...
        assertEquals(listOf(RmsBuildRangeCollapser.Run("[1,)", "1.8")), result)
    }
}
