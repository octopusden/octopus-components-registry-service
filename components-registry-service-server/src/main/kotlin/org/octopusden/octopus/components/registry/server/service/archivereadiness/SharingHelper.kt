...
class SharingHelper(
    ...
    fun sharedWithForRepo(canonicalUrl: String, excludeComponentId: UUID): List<String> =
        componentRepository.findAll()
            .filter { !it.archived && it.id != excludeComponentId }
            .filter { comp ->
                componentConfigurationRepository.findByComponentId(comp.id!!)
    ...
    fun sharedWithForTcProject(projectId: String, descendantIds: Set<String>, excludeComponentId: UUID): List<String> {
        ...
        return versionLineRepository.findByProjectIdsWithComponent(relevant.toList())
            .filter { vl -> !vl.component.archived && vl.component.id != excludeComponentId }
            .map { it.component.componentKey }
            .distinct()
    }

    fun sharedWithForJiraProject(projectKey: String, excludeComponentId: UUID): List<String> {
