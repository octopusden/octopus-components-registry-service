...
- [~] **RES-001 enumeration-coverage piece fixed; full re-enable blocked on externalRegistry.** Stream A introduced `row_type` as the source-of-truth row classifier on `component_configurations` and a new `RANGE_PRESENCE` row shape emitted by `ImportServiceImpl` for DSL ranges with no real override. `EntityMappers.toEscrowModule` enumerates presence rows via the existing `.distinct()` step; `resolveForRange` filters them out before scalar/marker overlay. V4 editor APIs hide presence rows. `GitVsDbValidationTest.VAL-006 maven-artifacts` re-enabled. `DbBackedComponentsRegistryServiceControllerTest.testGetAllJiraComponentVersionRanges` still @Disabled: 5 components diverge on per-DTO equality at `vcsSettings.externalRegistry` (null vs `NOT_AVAILABLE` default-emit); fix lives in the externalRegistry item below.
- [ ] **RES-008 `testGetDetailedComponent` — `escrow.buildTask` divergence.** The KTS override semantic stores `escrow.buildTask` and `build.buildTasks` as two distinct fields (Phase 5b folded the `escrow_build_task` column into V1__schema.sql). The v1 fixture was authored when the two were a single scalar, so `expected: "clean build -x test"` vs `got: "clean build"` is a fixture-drift, not a code bug. Rewrite the fixture to populate both fields, or split the assertion into separate per-field expectations.
...
- [ ] **FAKE-aggregator distribution routing edge.** Surfaces as 1× `core-lib` divergence in `VAL-010 bulk canary`. Distribution status emitted by the DB resolver differs from the Groovy baseline when the component sits behind a FAKE aggregator. Decide whether to align the FAKE branch on the resolver side or accept as v3-legacy.
- [ ] **`vcsSettings.externalRegistry: null` vs `NOT_AVAILABLE` default-emit divergence.** Surfaces as 1× `customer-gamma-wallet` divergence in `VAL-010`. The Groovy resolver emits an explicit `NOT_AVAILABLE` for components with no externalRegistry; the DB resolver emits `null`. Pick a canonical and apply consistently in `DatabaseComponentRegistryResolver`.
