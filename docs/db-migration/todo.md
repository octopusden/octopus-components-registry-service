...
- [ ] **Extract `PackageType` enum to the API module.** `EnumValidValues.PACKAGE_TYPE_NAMES` is the only hand-listed validation set in that file — derive from a real enum like the four siblings (`BuildSystem`, `EscrowGenerationMode`, `RepositoryType`, `ProductTypes`). See [TD-004](tech-debt/004-extract-package-type-enum.md).
