...
| B2.1 — /meta/labels + ?labels filter (SYS-040) | ✅ Done | Backend. `GET /rest/api/4/components/meta/labels` returns sorted distinct labelCodes from the component_labels junction. `GET /rest/api/4/components?labels=A,B` AND-filters via one join+predicate per code; CSV normalisation in the controller. Files: `ComponentLabelRepository.kt`, `ComponentControllerV4.kt`, `ComponentFilter.kt`, `ComponentManagementServiceImpl.kt` |
| B2.2 — ?buildSystem multi-value with OR semantics (SYS-041) | ✅ Done | Backend. `GET /rest/api/4/components?buildSystem=GRADLE,MAVEN` accepts CSV (and repeatable) input and returns the union (OR) over the BASE configuration row's buildSystem column. Specification swaps scalar `cb.equal` for `IN (...)`; same JOIN + rowType=BASE guard as before; controller reuses the labels normalisation pipeline. Files: `ComponentControllerV4.kt`, `ComponentFilter.kt`, `ComponentManagementServiceImpl.kt` |
| B2.3 — /meta/systems + ?system= multi-value with OR semantics (SYS-042) | ✅ Done | Backend. `GET /rest/api/4/components/meta/systems` returns sorted distinct system codes from the component_systems junction. `GET /rest/api/4/components?system=A,B` accepts CSV (and repeatable) input and OR-filters via one JOIN through systemJunctions + `IN(...)`. Files: `ComponentSystemRepository.kt`, `ComponentControllerV4.kt`, `ComponentFilter.kt`, `ComponentManagementServiceImpl.kt` |
| B2.4 — ?owner= multi-value with OR semantics (SYS-043) | ✅ Done | Backend. `GET /rest/api/4/components?owner=alice,bob` accepts CSV (and repeatable) input and OR-filters via a single `componentOwner IN (...)` predicate (scalar column, no JOIN). Files: `ComponentControllerV4.kt`, `ComponentFilter.kt`, `ComponentManagementServiceImpl.kt` |
