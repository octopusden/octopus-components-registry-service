...
- **Permissions** (8): `ACCESS_COMPONENTS`, `CREATE_COMPONENTS`, `ARCHIVE_COMPONENTS`, `RENAME_COMPONENTS`, `DELETE_COMPONENTS`, `IMPORT_DATA`, `ACCESS_AUDIT`, `EDIT_METADATA` (edit component-configuration metadata — gates the Portal Field-Overrides edit surface; `ROLE_ADMIN` only).
