...
- **Input**: Optional filters, ANDed when combined.
  ...
  - **Editors** (read-only "who can edit" projection, gated by `ACCESS_COMPONENTS`): `GET /rest/api/4/components/{idOrName}/editors` → `{ componentOwner, releaseManagers[], securityChampions[], manager }` — `manager` (`SYS-063`) is the owner's manager per employee-service, null when unresolvable. Informational only — administrators (`EDIT_ANY_COMPONENT`) may also edit any component but are, unlike the manager, not enumerated (an open-ended realm-role, not per-component data).
...
| `artifactIds[]` ownership mapping | `groupPattern` + each EXPLICIT `artifactTokens[]` entry must match the literal allowlist `[A-Za-z0-9_.-]+` (no wildcards/regex); `EXPLICIT` requires ≥1 token, `ALL`/`ALL_EXCEPT_CLAIMED` require none; `ALL_EXCEPT_CLAIMED` is single-group; `versionRange` must equal an existing config range or be the base | Validated on create and the PATCH REPLACE path. See §2.5 (mode-aware ownership). Unspecified mode defaults to `ALL` when tokenless, else `EXPLICIT`. |
...
Conflicts with **other** components → **409 Conflict** (`CrossComponentConflictException`). Every uniqueness 409 carries a message starting with `uniqueness violation:` and `errorCode: "UNIQUENESS_VIOLATION"` in the `ErrorResponse` body (optimistic-lock 409s carry `errorCode: "OPTIMISTIC_LOCK"` instead) so clients — the Portal in particular — can distinguish "fix the conflicting value" from "reload and re-apply":
- **Duplicate maven artifact rows in overlapping ranges** — two components must not claim the same maven coordinate for intersecting version ranges, where the coordinate identity is the FULL `(groupPattern, artifactPattern, extension, classifier)` tuple (`MavenGavCollision`). Differing extension or classifier is NOT a collision: `g:a:zip` and `g:a:apk` are distinct artifacts. A `null` extension/classifier only matches `null` — it is its own identity, not a wildcard. Group/artifact match under the UNION of the two legacy `EscrowConfigValidator` rules the production DSL is authored against: **#25 pattern containment** (the row's WHOLE group string vs the rival's comma-split items — CSV-vs-CSV never matches, `|` is not a separator — plus `*`/regex artifact matching) and **#24 exact token-pair sharing** (groups split by `,`, artifact patterns by the legacy `[,|\s]+`, tokens compared literally — this is what catches identical-CSV duplicates). The check covers explicit `distribution { GAV }` rows; the groupId/artifactId **ownership** mapping has its own mode-aware check (next bullet). Intersection uses the same Maven boundary semantics as section 2.1.
- **Artifact-ownership mapping uniqueness (mode-aware — restores legacy #24/#25)** — the base groupId/artifactId ownership mapping is modelled explicitly as `component_artifact_mappings` (replacing the opaque regex `component_artifact_ids` and the import-only `group-artifact-pattern` marker). A component owns a LIST of mappings; each mapping = exactly **one** Maven groupId + an ownership **mode** (`EXPLICIT` literal tokens / `ALL_EXCEPT_CLAIMED` catch-all-that-yields / `ALL` greedy) for a version range (a legacy comma group-list `"a,b"` is CANONICALIZED — split at import and on the v4 write path into one row per group sharing the same mode/tokens/range, so `group_pattern` never stores a comma) (base = ALL_VERSIONS; a per-range override REPLACES base for that range — most-specific wins). Cross-component uniqueness is decided **deterministically from the stored modes** (no probe/regex heuristics): for two mappings of different components sharing ≥1 group token in intersecting ranges — `EXPLICIT × EXPLICIT` conflicts iff their literal token sets intersect; `EXPLICIT × ALL_EXCEPT_CLAIMED` never conflicts (the catch-all yields); `ALL_EXCEPT_CLAIMED × ALL_EXCEPT_CLAIMED` conflicts (two fallback owners); `ALL × anything` conflicts. Enforced on **create** and on **update when `artifactIds` is present** (not on unrelated field-overrides — they cannot change ownership, and re-checking would 409 on pre-existing data). Write-time invariants (400): group/token allowlist `[A-Za-z0-9_.-]+`; `EXPLICIT` requires ≥1 token while `ALL`/`ALL_EXCEPT_CLAIMED` carry none; `ALL_EXCEPT_CLAIMED` is single-group; an ownership range must equal an existing configuration range (or ALL_VERSIONS); per-component non-base ownership ranges must be disjoint; ≤1 mapping per group token per `(component, range)`. Migration classifies each DSL artifactId into a mode strictly (`(?!…)`→ALL_EXCEPT; exact catch-all forms→ALL; literal enumeration→EXPLICIT; any other regex hard-fails — no escape hatch). v1–v3 wire RE-COMPOSES the per-groupId rows of a range back into the legacy single `(groupIdPattern, artifactIdPattern)` pair — contiguous split-equivalent rows (same mode + tokens) starting at the PRIMARY (lowest `sort_order`) are re-joined into the comma group-list (`ArtifactOwnershipGrouping`), so `/maven-artifacts`, the escrow view and view-as-code stay byte-identical to legacy; genuinely-distinct same-range mappings and `ALL_EXCEPT_CLAIMED` rows never merge (mode→pattern, EXPLICIT tokens dot-escaped); reverse find-by-artifact flattens ALL mappings. The production model/api component families (literal placeholders vs the inherited `ANY_ARTIFACT` catch-all, now `ALL`/`ALL_EXCEPT_CLAIMED`) stay legal. **Scope note:** the cross-component ownership matrix is enforced on the v4 write path; the §6.0 migration pre-pass does NOT hard-fail on ownership (production is overlap-free by construction under the legacy resolver, and the live API guards new writes).
- **Jira `(projectKey, versionPrefix)` uniqueness among non-archived components** — each EFFECTIVE `(projectKey, versionPrefix)` pair maps to at most one non-archived component. Claims are the MERGED per-range values (`computeEffectiveJiraPairs`): per-range scalar overrides layer over the BASE row, so a range that overrides only `projectKey` claims `(overrideKey, inherited base prefix)` — NOT `(overrideKey, null)` (bucketing raw rows falsely conflicted with legitimate no-prefix owners — a real production shape). `overriddenAttribute` — not value nullness — decides whether a scalar was overridden, so a null-clear prefix override claims `(projectKey, null)`. A `null` version prefix is its own bucket. Archived components are exempt and do not claim a bucket.
...
- **Component-name uniqueness on rename** — renaming to an existing component key is rejected (`ComponentNameConflictException`, also `UNIQUENESS_VIOLATION`). Duplicate name/displayName on **create** stays a field-prefixed **400** (`name: …` / `displayName: …`) by design — the Portal routes those inline onto the form field.
...
- **Contract**: `SYS-095` in [requirements-common.md](requirements-common.md); full behavior in [openspec/specs/component-archive-readiness/spec.md](../../openspec/specs/component-archive-readiness/spec.md).
