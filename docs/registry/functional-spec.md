...
- **Input**: Optional filters, ANDed when combined.
  ...
  - **Editors** (read-only "who can edit" projection, gated by `ACCESS_COMPONENTS`): `GET /rest/api/4/components/{idOrName}/editors` → `{ componentOwner, releaseManagers[], securityChampions[] }`. Informational only — administrators (`EDIT_ANY_COMPONENT`) and the owner's manager (`SYS-063`) may also edit but are not enumerated.
