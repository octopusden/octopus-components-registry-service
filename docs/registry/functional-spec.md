...
- **Duplicate distribution GAV in overlapping ranges** — two components must not declare the same maven coordinate for intersecting version ranges, where the coordinate identity is the FULL `(groupPattern, artifactPattern, extension, classifier)` tuple (`MavenGavCollision`). Differing extension or classifier is NOT a collision: `g:a:zip` and `g:a:apk` are distinct artifacts (two components legitimately publish the same `g:a` as zip and apk). A `null` extension/classifier only matches `null` — it is its own identity, not a wildcard. Group/artifact compare with legacy pattern semantics (CSV/pipe groups, `*`/regex/CSV artifacts); intersection uses the same Maven boundary semantics as section 2.1.
