...
- `GET /rest/api/4/components/meta/employees?search=<q>` → `[{username, active}]` — an exact `getEmployee` probe (0/1 result); the employee-service client has no prefix search, so typeahead suggestions come from `/meta/owners` and this annotates the active flag.
