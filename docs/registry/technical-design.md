...
- **Unified VCS model.** SINGLE-VCS = 1 entry in `vcs_settings_entries` with `name = NULL`; MULTI-VCS = N named entries. Entries carry optional placement (`source_path`, `checkout_directory`, ONB-001): each entry is checked out under its `checkout_directory`, which is also its name, and at most one entry of a row goes without one, at the checkout root; the row's `build_working_directory` says where the build runs. Validated in `replaceVcsEntries` (functional-spec §1.4).
