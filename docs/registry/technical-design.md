...
- **Unified VCS model.** SINGLE-VCS = 1 entry in `vcs_settings_entries` with `name = NULL`; MULTI-VCS = N named entries. Entries carry optional placement (`source_path`, `checkout_directory`, ONB-001): the primary (`sort_order` 0) is checked out at the checkout root, each secondary under its `checkout_directory`, which is also its name; validated in `replaceVcsEntries` (functional-spec §1.4).
