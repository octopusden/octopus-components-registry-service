...
| `sort_order` | INT NOT NULL DEFAULT 0 | `0..n-1` per row; 0 = primary entry |
| `source_path` | VARCHAR(255), nullable | Repository directory that belongs to the component; NULL = whole repository (`V8__`) |
| `checkout_directory` | VARCHAR(255), nullable | Directory the entry is checked out to; NULL on the primary (checkout root), required on secondaries by the v4 write path (`V8__`) |
...
rule to the rows it creates. Names are copied verbatim, so a row whose secondary names are not valid
or not distinct checkout directories stays readable and fails its next v4 VCS write with `400`. On
v4 writes `name` is derived: a secondary's is its `checkout_directory`, the primary's the previous
primary's name (`main` for a row without entries). Placement rules: functional-spec §1.4.
