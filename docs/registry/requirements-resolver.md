...
- **Per-attribute values.** The BASE row is **always** at `ALL_VERSIONS`. Its VALUE is the effective config of the **OPEN-UPPER (newest) range** — the value current versions resolve to — **not** `top-level ⊕ Defaults` (base-row semantics superseded 2026-07; see [ADR-018 §2 amendment](adr/018-decoupled-version-model.md)). The OLDER declared blocks become the overrides. There is **no synthetic-bounded base** (`is_synthetic_base` is vestigial / always `false`). Per-attribute overrides apply on ranges **including open-upper** (`[X,)`), selected by **containment** (TD-010), not exact range match.
