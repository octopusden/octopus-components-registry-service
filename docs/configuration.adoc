...
== Overview
Components Registry is a registry of all link:{glossary-component-link}[components] developed in company.
...
----
<component id> {
        componentDisplayName = "Name of component" //  DMS description
        parentComponent      = <component id> // grouping components in DMS
        clientCode           = <client code> // if component is client specific
        solution             = <true|false> // true - meta-component

        componentOwner   = "username of component's owner" //without domain part. For example: salekseev
        releaseManager   = "username of component's release manager" 
        securityChampion = "username of component's security officer" 
        ...
        jira { // Parameters releated to build/release registration in JIRA and generation of client release notes (default for all versions)
            ...
            component {
               versionPrefix = "prefix in component versioning" // to simplify choosing component version in Fix Version(s) field of Jira issue
        ...
        distribution { // Distribution parameters (default for all versions)
            ...
            external = <true|false> // true - can be published
