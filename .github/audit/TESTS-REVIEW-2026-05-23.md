...
1. **Build cache + configuration cache**. Lowest risk, easiest to validate (one CI run with vs without, compare TC timings on identical commits). Expected: warm-cache savings on repeat builds, no first-build benefit.
...
| F | `chore/build-cache-quick-wins` (optional, after user signal) | Add `org.gradle.caching=true`, `org.gradle.configuration-cache=true` to `gradle.properties`. Validate with one CI run on the same commit. No test changes. | 1 | low | independent |
