...
class ReleaseInfoResolverTest {
    ...
    void testExcludedDistributionValidationConfigurationInvalid() {
        ...
        assertTrue("Expected validation error message was not found.", exception.message.contains("External explicitly distributed components for version range '(,0),[0,)' must define at least one distribution coordinate (distribution->GAV, DEB, RPM, Docker, or generic) in 'component_test'."))
    }
