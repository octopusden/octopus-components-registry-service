...
- **ArchUnit** architecture fitness functions in `components-registry-service-server` (`server/architecture/ArchitectureFitnessTest.kt`, runs in the fast `test` gate). Pre-existing violations of the frozen rules (controllers → repository; v4 endpoints without `@PreAuthorize`) are baselined in `archunit_violation_store/` — treat it like `detekt-baseline.xml`: **new** violations fail the build; regenerate only to prune fixed entries, never to absorb new ones (`freeze.refreeze=false`). Caveat: a baseline is keyed by the rule's full description, and `allowStoreUpdate=true` means renaming a frozen rule (editing its description/`.because(...)`) **silently re-records** a fresh baseline under the new key rather than failing — so a rename can quietly absorb a regression. When you change a frozen rule's text, delete the orphaned old key + file, regenerate, and verify the new baseline. See `docs/registry/non-functional-spec.md` §5.3.
