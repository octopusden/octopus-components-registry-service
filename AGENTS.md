...
- JaCoCo coverage — aggregate LINE ≥ 86% / BRANCH ≥ 65%, plus strict per-module LINE/BRANCH **floors** on top of the plugin-owned 10% floor. In `qualityCoverage` only, so TeamCity `[1.0]` does not gate coverage. Floors are set to measured − ~2 p.p.: raise them freely, and lowering one requires saying so in the PR — that is a review rule, not a build rule, since nothing in the build stops a lowered number. `branch` is optional per module (line-only is legitimate where the code has no meaningful branching). Rules and rationale: `build.gradle` (`moduleCoverageFloors`, `verifyCoverageFloorsPolicy`) and `docs/registry/non-functional-spec.md` §5.10; measured values and re-measure recipe: `docs/registry/quality-baseline.md`.
