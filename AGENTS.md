...
- **Mutation testing (PIT)** over `..server.util..` / `..server.mapper..` — `./gradlew :components-registry-service-server:pitest`, ratchets `mutationThreshold = 53` / `coverageThreshold = 78`. Non-gating `mutation.yml`; not in `check`/`build`/`qualityCoverage`. The only gate that fails an assertion-free test. Adding logic to those packages means killing your own mutants. Scope, the two load-bearing exclusions, and why they matter: `components-registry-service-server/build.gradle` (`pitest {}` block).
