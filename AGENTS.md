...
- JaCoCo coverage — an aggregate LINE/BRANCH floor plus per-module LINE/BRANCH floors (`branch` optional, since line-only is legitimate where code has no meaningful branching), on top of the plugin-owned per-module floor. Enforced in `qualityCoverage` only, so TeamCity `[1.0]` does not gate coverage. Floors sit a little below the measured values: raise them freely, and lowering one requires saying so in the PR — a review rule, not a build rule, since nothing in the build stops a lowered number. The numbers live in `build.gradle` and nowhere else, in two places: the aggregate minimums in `jacocoOverallCoverageVerification`'s violation rules, the per-module ones in the `moduleCoverageFloors` map — which `verifyCoverageFloorsPolicy` validates. Rationale: `docs/registry/non-functional-spec.md` §5.10; measurements and the re-measure recipe: `docs/registry/quality-baseline.md`.
- **Mutation testing (PIT)** over `..server.util..` / `..server.mapper..` — `./gradlew :components-registry-service-server:pitest`. The task fails below a mutation-score floor or a coverage floor for the targeted classes; both values live in the `pitest {}` block and nowhere else. Floors, not ratchets: lowering one is only stopped by review. A **check, not a gate** — `mutation.yml` reports on the PR but is not in `gate-merge.needs`; not in `check`/`build`/`qualityCoverage`. The only check that surfaces tests which exercise code without checking the result — a **surviving** mutant means nothing noticed the change; a killed one only means something did (possibly an incidental exception). Adding logic to those packages means killing your own mutants. Scope, the two load-bearing exclusions, and why they matter: `components-registry-service-server/build.gradle` (`pitest {}` block).
...
- Gate BRANCH as well as LINE: LINE alone cannot see a conditional taken on only one path. Note what
  neither counter can see — whether the result was checked at all; an assertion-free test can reach 100% on
  both. That gap is what the mutation check helps surface (see Quality Gates above).
