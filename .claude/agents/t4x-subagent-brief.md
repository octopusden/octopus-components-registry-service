...
**CRS image to use for FT:** `2.0.84-3122` — a TeamCity-published branch snapshot of
`feature/ft-db-testing`. Pull from the team's registry (e.g.
`ghcr.io/octopusden/components-registry-service:2.0.84-3122` or the internal Docker
...
`2.0.84-3122` only for your local FT run — either via env var override or a local-only
...
- Image env: `OCTOPUS_COMPONENTS_REGISTRY_SERVICE_VERSION` — for the FT run, override to the
  TC-published branch snapshot `2.0.84-3122`. Do not hardcode this value in committed files;
